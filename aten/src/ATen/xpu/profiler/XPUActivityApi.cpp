...
void XPUActivityApi::pushCorrelationID(int id, CorrelationFlowType type) {
  ...
  switch (type) {
    case Default:
      XPU_PTI_CHECK(ptiViewPushExternalCorrelationId(
...
void XPUActivityApi::popCorrelationID(CorrelationFlowType type) {
  ...
  switch (type) {
    case Default:
      XPU_PTI_CHECK(ptiViewPopExternalCorrelationId(
...
std::unique_ptr<XPUActivityBufferMap> XPUActivityApi::activityBuffers() {
  ...
  XPU_PTI_CHECK(ptiFlushAllViews());
  std::lock_guard<std::mutex> guard(mutex_);
...
void XPUActivityApi::enablePtiActivities(
  ...
  XPU_PTI_CHECK(ptiViewSetCallbacks(
      bufferRequestedTrampoline, bufferCompletedTrampoline));

  externalCorrelationEnabled_ = false;
  for (const auto& activity : selected_activities) {
    if (activity == act_t::GPU_MEMCPY) {
      XPU_PTI_CHECK(ptiViewEnable(PTI_VIEW_DEVICE_GPU_MEM_COPY));
    }
    if (activity == act_t::GPU_MEMSET) {
      XPU_PTI_CHECK(ptiViewEnable(PTI_VIEW_DEVICE_GPU_MEM_FILL));
    }
    if (activity == act_t::CONCURRENT_KERNEL) {
      XPU_PTI_CHECK(ptiViewEnable(PTI_VIEW_DEVICE_GPU_KERNEL));
    }
    if (activity == act_t::EXTERNAL_CORRELATION) {
      XPU_PTI_CHECK(ptiViewEnable(PTI_VIEW_EXTERNAL_CORRELATION));
      externalCorrelationEnabled_ = true;
    }
    if (activity == act_t::XPU_RUNTIME) {
      XPU_PTI_CHECK(ptiViewEnable(PTI_VIEW_SYCL_RUNTIME_CALLS));
    }
    if (activity == act_t::OVERHEAD) {
      XPU_PTI_CHECK(ptiViewEnable(PTI_VIEW_COLLECTION_OVERHEAD));
    }
  }
...
void XPUActivityApi::disablePtiActivities(
  ...
  for (const auto& activity : selected_activities) {
    if (activity == act_t::GPU_MEMCPY) {
      XPU_PTI_CHECK(ptiViewDisable(PTI_VIEW_DEVICE_GPU_MEM_COPY));
    }
    if (activity == act_t::GPU_MEMSET) {
      XPU_PTI_CHECK(ptiViewDisable(PTI_VIEW_DEVICE_GPU_MEM_FILL));
    }
    if (activity == act_t::CONCURRENT_KERNEL) {
      XPU_PTI_CHECK(ptiViewDisable(PTI_VIEW_DEVICE_GPU_KERNEL));
    }
    if (activity == act_t::EXTERNAL_CORRELATION) {
      XPU_PTI_CHECK(ptiViewDisable(PTI_VIEW_EXTERNAL_CORRELATION));
    }
    if (activity == act_t::XPU_RUNTIME) {
      XPU_PTI_CHECK(ptiViewDisable(PTI_VIEW_SYCL_RUNTIME_CALLS));
    }
    if (activity == act_t::OVERHEAD) {
      XPU_PTI_CHECK(ptiViewDisable(PTI_VIEW_COLLECTION_OVERHEAD));
    }
  }
  externalCorrelationEnabled_ = false;
}
